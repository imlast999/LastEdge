                    ...
                    try:
                        ...
                        if self.path in ('/', '/dashboard'):
                            ...
                            try:
                                ...
                                pass
                        elif self.path.startswith('/api/system/backup/list'):
                            from services.bot_service import get_bot_service
                            data = get_bot_service().list_backups()
                            body = json.dumps({"ok": True, "backups": data}, indent=2, default=str).encode('utf-8')
                            self.send_response(200)
                            self.send_header('Content-type', 'application/json')
                            self.send_header('Access-Control-Allow-Origin', '*')
                            self.end_headers()
                            try:
                                ...
                                pass
                        elif self.path.startswith('/api/system/stability-verification'):
                            from services.bot_service import get_bot_service
                            data = get_bot_service().run_stability_verification()
                            body = json.dumps(data, indent=2, default=str).encode('utf-8')
                            self.send_response(200)
                            self.send_header('Content-type', 'application/json')
                            self.send_header('Access-Control-Allow-Origin', '*')
