"""
run_exit_research_xauusd.py — Exit Research para la estrategia activa de XAUUSD

Uso:
    python run_exit_research_xauusd.py
    python run_exit_research_xauusd.py --bars 10000    # análisis rápido
    python run_exit_research_xauusd.py --bars 20000    # completo (recomendado)
...
Protocolo idéntico al de EURUSD — mismas métricas, mismas variantes, mismo pipeline.
"""
...
# Estrategias XAUUSD descartadas — se excluyen automáticamente
DISCARDED_XAUUSD = {
    "xauusd_advanced": "Fallback no activo en producción",
    "xauusd_reversal": "No validada — experimental",
}


# ── Detectar estrategia XAUUSD activa ────────────────────────────────────────

def get_active_xauusd_strategy() -> List[Dict[str, Any]]:
    """
    Lee rules_config.json y devuelve la estrategia XAUUSD activa.

    Returns:
        Lista con un dict: name, sl_mult, tp_mult, label, source
    """
    try:
    ...
    xauusd_cfg = cfg.get("XAUUSD", {})
    if not xauusd_cfg.get("enabled", True):
        logger.warning("XAUUSD está desactivado en rules_config.json")
        return []

    primary = xauusd_cfg.get("strategy", "xauusd_simple")

    # Leer multiplicadores: si no están en rules_config, usar defaults de la estrategia
    from strategies.xauusd import XAUUSDStrategy
    _defaults = XAUUSDStrategy()._get_default_config()
    sl_mult = xauusd_cfg.get("sl_atr_multiplier", _defaults.get("sl_atr_multiplier", 2.0))
    tp_mult = xauusd_cfg.get("tp_atr_multiplier", _defaults.get("tp_atr_multiplier", 5.0))

    strategies = []
    if primary not in DISCARDED_XAUUSD:
        strategies.append({
            "name":    primary,
            "sl_mult": sl_mult,
            "tp_mult": tp_mult,
            "label":   f"{primary} (producción)",
            "source":  "rules_config.json › XAUUSD.strategy",
        })
    else:
        logger.warning(
            "Estrategia principal XAUUSD '%s' marcada como descartada: %s",
            primary, DISCARDED_XAUUSD.get(primary, ""),
        )
...
def print_strategy_audit(strategies: List[Dict], discarded: Dict) -> None:
    ...
    print()
    print("=" * 72)
    print("  AUDIT DE ESTRATEGIAS XAUUSD")
    print("=" * 72)
    print()
    print("  INCLUIDAS:")
    for s in strategies:
...
def build_production_variant(sl_mult: float, tp_mult: float):
    ...
    de producción de rules_config.json para XAUUSD.
...
def run_research_for_strategy(
    ...
    """
    Ejecuta el pipeline completo de Exit Research para la estrategia XAUUSD.
    Reutiliza exactamente la misma infraestructura que EURUSD.
    ...
    try:
        from strategies.xauusd import XAUUSDStrategy
        base_strategy = XAUUSDStrategy()
    except Exception as e:
        logger.error("No se pudo instanciar XAUUSDStrategy: %s", e)
        raise
    ...
    # Variante de producción + todas las variantes estándar
    prod_variant = build_production_variant(sl_mult, tp_mult)
    all_variants = [prod_variant] + list(ALL_VARIANTS)

    # ExitResearchRunner con symbol=XAUUSD — inyecta pip_size automáticamente
    ...
    logger.info(
        "Iniciando Exit Research XAUUSD: strategy=%s  bars=%d  variantes=%d",
...
def analyze_report(report: Dict, strategy_info: Dict) -> str:
    ...
    Igual que en EURUSD — mismas métricas, misma estructura.
    """
    strategy_name = strategy_info["name"]
    sl_mult       = strategy_info["sl_mult"]
    tp_mult       = strategy_info["tp_mult"]
    ...
    lines = []
    lines.append(f"# Análisis Exit Research — XAUUSD / {strategy_name}")
    lines.append(f"")
    ...
    lines.append(f"**Nota:** Métricas en pips XAUUSD (1 pip = $0.01). "
                 f"ATR H1 XAUUSD ≈ 200–500 pips.")
    lines.append(f"")
...
def analyze_report_level2(report: Dict, strategy_info: Dict) -> str:
    ...
    # Interpretación cuantitativa (adaptada a XAUUSD)
    lines.append("### Interpretación cuantitativa (XAUUSD)")
    lines.append("")
    lines.append("_Nota: métricas en pips XAUUSD. ATR H1 ≈ 200–500 pips. "
                 "XAUUSD desarrolla tendencias largas — las variantes trailing "
                 "tienen ventaja natural en este símbolo._")
    lines.append("")

    # TP demasiado ambicioso
    if prod_cap < 40:
        lines.append(f"- **TP demasiado ambicioso:** `profit_captured_pct={prod_cap:.1f}%`. "
                     "Las ganadoras cierran capturando menos del 40% del movimiento máximo. "
                     "En XAUUSD con tendencias largas, un trailing puede capturar más.")
    elif prod_cap < 60:
        lines.append(f"- **TP moderado:** `profit_captured_pct={prod_cap:.1f}%`. "
    ...
    # ATR XAUUSD H1 ≈ 2-5 USD = 200-500 pips
    if prod_mae > 0 and sl_mult > 0:
        sl_pips_est = sl_mult * 300  # estimación: ATR ≈ 300 pips H1
        if prod_mae < sl_pips_est * 0.5:
            lines.append(f"- **SL posiblemente amplio:** MAE medio en perdedoras "
                         f"({prod_mae:.0f} pips) notablemente menor que el SL estimado "
                         f"({sl_pips_est:.0f} pips). El precio rara vez alcanza el SL completo.")
        else:
            lines.append(f"- **SL utilizado eficientemente:** MAE medio en perdedoras "
                         f"({prod_mae:.0f} pips) cercano al SL configurado.")

    # Duración media
    if prod_dur > 0:
        hours = prod_dur
        if hours > 72:
            lines.append(f"- **Trades de larga duración:** {prod_dur:.0f} velas H1 "
                         f"(≈ {hours/24:.1f} días). En XAUUSD esto puede ser normal "
                         "dado que las tendencias son prolongadas.")
        elif hours < 8:
            lines.append(f"- **Trades muy cortos:** {prod_dur:.0f} velas H1. "
                         "El sistema cierra muy rápido — los trailings podrían "
                         "no capturar las tendencias características de XAUUSD.")
        else:
            lines.append(f"- **Duración normal:** {prod_dur:.0f} velas H1 (≈ {hours:.0f}h).")

    # Win rate
    if prod_wr < 30:
        lines.append(f"- **Win Rate bajo ({prod_wr:.1f}%):** el sistema depende del RR alto "
                     f"(RR≈1:{tp_mult/sl_mult:.1f}). Necesita WR > "
                     f"{1/(1+tp_mult/sl_mult)*100:.0f}% para breakeven.")
    elif prod_wr > 55:
        lines.append(f"- **Win Rate alto ({prod_wr:.1f}%):** el sistema gana frecuentemente. "
                     "En XAUUSD, un trailing podría capturar más de los grandes movimientos.")
    else:
...
def potential_improvements(report: Dict, strategy_info: Dict) -> str:
    ...
    if better_rr:
        ...
        lines.append(f"   → En XAUUSD el mercado puede alcanzar un TP más cercano antes de revertir.")
        lines.append("")
        improvements_found += 1

    # 2. Trailing ATR (especialmente relevante para XAUUSD)
    ...
    if atr_row and float(atr_row.get("stability_score") or 0) > prod_stab + 3:
        ...
        lines.append(f"   → XAUUSD desarrolla tendencias largas. "
                     "Sin TP fijo captura mejor los grandes movimientos.")
        lines.append("")
        improvements_found += 1

    # 3. Donchian (también idóneo para XAUUSD)
    don_row = next((r for r in comp if r.get("variant") == "trailing_donchian"), None)
    if don_row and float(don_row.get("stability_score") or 0) > prod_stab + 3:
        diff = float(don_row.get("stability_score") or 0) - prod_stab
        bm   = m("trailing_donchian")
        lines.append(f"3. **Trailing Donchian**  _(Stability +{diff:.1f} pts vs producción)_  ")
        lines.append(f"   PF={fmt_n(don_row.get('profit_factor'))}  "
                     f"Cap%={fmt_n(bm.get('profit_captured_pct'),1)}%  "
                     f"MaxDD={fmt_n(don_row.get('max_drawdown'),1)}")
        lines.append(f"   → Trailing adaptativo: más ajustado en mercados con rango reducido.")
        lines.append("")
        improvements_found += 1
    ...
    if be_row:
        ...
        if be_mae_l < prod_mae_l * 0.8:
            lines.append(f"4. **Break Even automático**  "
                         f"_(MAE perdedoras: {be_mae_l:.0f} vs {prod_mae_l:.0f} pips actual)_  ")
            lines.append(f"   → Reduce la pérdida media en operaciones que rebotan antes del SL.")
            lines.append("")
            improvements_found += 1
...
def main():
    parser = argparse.ArgumentParser(
        description="Exit Research — análisis cuantitativo de salidas XAUUSD"
    )
    parser.add_argument(
    ...
    print(f"  Pip size: $0.01 por pip (XAUUSD H1)")
    print("=" * 72)

    # Detectar estrategia activa
    strategies = get_active_xauusd_strategy()
    print_strategy_audit(strategies, DISCARDED_XAUUSD)

    if not strategies:
        logger.error("No hay estrategia XAUUSD activa para analizar.")
        sys.exit(1)
